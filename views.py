...
class View:
    ...
    def agenda_visualizar(id , dataini_str, datafin_str):
        ...
        lista = []
        for agenda in View.agenda_listar():
            if agenda.get_id_cliente() == id and dataini.date() <= agenda.get_data().date() <= datafin.date():
                lista.append(agenda)
        return lista
    
    def agenda_confirmar(id):
        for agenda in View.agenda_listar():
            if id == agenda.get_id():
                NAgenda.atualizar(Agenda(agenda.get_id(), agenda.get_data(), True, agenda.get_id_cliente(), agenda.get_id_servico()))
